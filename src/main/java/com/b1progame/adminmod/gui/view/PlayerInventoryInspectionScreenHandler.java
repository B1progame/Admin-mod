...
public final class PlayerInventoryInspectionScreenHandler extends GenericContainerScreenHandler {
    private static final int SLOT_BACK = 49;
    private final ServerPlayerEntity viewer;
    private final AdminGuiService guiService;
    private final UUID targetUuid;
    private final TargetPlayerInventoryView inventoryView;

    public PlayerInventoryInspectionScreenHandler(int syncId, PlayerInventory playerInventory, AdminGuiService guiService, UUID targetUuid) {
        super(ScreenHandlerType.GENERIC_9X6, syncId, playerInventory, new TargetPlayerInventoryView((ServerPlayerEntity) playerInventory.player, guiService, targetUuid), 6);
        this.viewer = (ServerPlayerEntity) playerInventory.player;
        this.guiService = guiService;
        this.targetUuid = targetUuid;
        this.inventoryView = (TargetPlayerInventoryView) this.getInventory();
        this.getInventory().setStack(SLOT_BACK, GuiItemFactory.button(Items.BARRIER, "Back", Formatting.RED, List.of("Return to inventory menu")));
    }
    ...
    public void onClosed(PlayerEntity player) {
        ...
        if (target != null) {
            ...
            AuditLogger.inventory(
                    ...
                    AuditLogger.actor(this.viewer) + " closed inventory view for " + target.getGameProfile().name() + " (" + target.getUuidAsString() + ")"
            );
            return;
        }
